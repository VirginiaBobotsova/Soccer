...
export class CategoryService {
    ...
    private createCategories (events: SoccerEvent[]) {
        ...
        const uniqueCategoryRank = [...new Set(events.map(item => item.categoryRank))]
            .sort((a, b) => b - a);
        const uniqueLeagueRank = [...new Set(events.map(item => item.leagueRank))]
            .sort((a, b) => b - a);
        ...
        uniqueCategoryRank.forEach(cr => {
            uniqueLeagueRank.forEach(lr => {
                ...
                events.forEach(event => {
                    if (event.categoryRank ===  category.categoryRank
                        && event.leagueRank ===  category.leagueRank) {
