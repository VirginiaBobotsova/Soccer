...
@Injectable({
   ...
   export class SignalRService {
    ...
    public updateMarket0(): void {
        if (this.connection) {
            this.connection.invoke('SubscribeToGroup', this.channel0)
                .then(res => {
    ...
    public updateMarket1() {
        if (this.connection) {
            this.connection.invoke('SubscribeToGroup', this.channel1)
                .then(res => {
                    console.log('Successfully subscribed', res.json());
                })
                .catch((error: any) => {
    ...
    public updateMarket2() {
        if (this.connection) {
            this.connection.invoke('SubscribeToGroup', this.channel2)
            .then(res => {
                console.log('Successfully subscribed', res.json());
            })
            .catch((error: any) => {
